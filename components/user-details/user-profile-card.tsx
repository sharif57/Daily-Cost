...
// import { useDeleteDocumentMutation, useSingleUserQuery, useUpdateDocumentMutation } from '@/redux/feature/userSlice'
...
import Link from 'next/link'
import { Mail, Phone, CreditCard as IdentityCard, Calendar, FileText, Trash2, Plus, X } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { useDeleteDocumentMutation, useSingleUserQuery, useUpdateDocumentMutation } from '@/redux/feature/userSlice'
import { useParams } from 'next/navigation'
import { useState } from 'react'
...
const getDocumentUrl = (docPath: string, apiBase: string): string => {
  if (!docPath?.trim()) return ''
  return docPath.startsWith('http') ? docPath : `${apiBase}${docPath}`
}

const getDocumentName = (docPath: string): string => {
...
export default function UserProfileCard() {
  ...
  // document is now an array
  const documents: string[] = Array.isArray(user?.document) ? user.document : []
  ...
  return (
    <div className="bg-card rounded-lg border border-border p-6 sticky top-6">
      {/* User Avatar */}
      <div className="flex flex-col items-center mb-6">
        <div className="w-24 h-24 rounded-full bg-gradient-to-br from-cyan-300 to-cyan-500 flex items-center justify-center text-white font-bold text-3xl mb-2 relative overflow-hidden">
          {profileImage ? (
            <img
              src={profileImage}
              alt={user?.name ?? 'User avatar'}
            ...
            {hasDocuments ? (
              <div className="space-y-2">
                {documents.map((docPath, index) => {
                  const docUrl = getDocumentUrl(docPath, API_BASE)
                  const docName = getDocumentName(docPath)
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="text-xs font-medium text-[#090A58] hover:underline truncate"
                          title={docName}
                        >
                          {docName}
                        </a>
                      ...
                      <button
                        ...
                        disabled={isDeleting}
                        className="shrink-0 text-red-500 hover:text-red-700 disabled:opacity-50 transition-colors"
      ...
      {isDocumentModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-2xl rounded-2xl border border-border bg-card p-6 shadow-lg">
            <div className="mb-6 flex items-center justify-between">
            ...
            <div className="space-y-4">
              {/* Upload new */}
              <div className="space-y-3">
                <p className="text-sm font-medium text-foreground">Select File</p>
                <input
                  type="file"
                  accept=".pdf,.doc,.docx,.png,.jpg,.jpeg"
                  onChange={handleDocumentFileChange}
